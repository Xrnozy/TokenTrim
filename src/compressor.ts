...
export type CompressionLevel = 'light' | 'balanced' | 'aggressive'; // Legacy API support
...
export interface CompressedContext {
  ...
  timestamp: number;
  fileCount?: number; // For legacy API compatibility
  mode?: ReadMode;
  cacheKey?: string;
  cacheHit?: boolean;
}
...
export class ContextCompressor {
  ...
  public symbolMap: Map<string, string> = new Map(); // Track symbol compressions for reversibility

  // ============ LEGACY API (Main Entry Point) ============

  /**
   * Legacy compress() method for backward compatibility
   * Maps old API (content, level) to new mode-based approach
   */
  public compress(
    content: string,
    ...
    // Map compression levels to read modes
    let mode: ReadMode;
    switch (level) {
      case 'light':
        mode = 'full'; // Minimal compression, everything intact
        break;
      case 'balanced':
        mode = 'signatures'; // Good balance of compression and preservation
        break;
      case 'aggressive':
        mode = 'aggressive'; // Maximum compression, remove boilerplate
        break;
      default:
        mode = 'signatures';
    }
    ...
    if (this.cache.has(cacheKey) && !isEditing) {
      ...
      if (!this.isCacheExpired(cached)) {
        return {
          ...
          metrics: this.calculateMetrics(cached.content, content),
  ...
  private removeAnsiCodes(content: string): string {
    return content.replace(
      new RegExp(`[\\u001b\\u009b][[()#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-ORZcm]`, 'g'),
      ''
    );
  }
  ...
  private calculateMetrics(
    ...
    // Token counting: hybrid approach (words × 1.3 + bytes / 8)
    const originalTokens = Math.ceil(originalWords * 1.3 + originalBytes / 8);
    const compressedTokens = Math.ceil(compressedWords * 1.3 + compressedBytes / 8);

    return {
      original: { lines: originalLines, tokens: originalTokens, bytes: originalBytes },
      compressed: { lines: compressedLines, tokens: compressedTokens, bytes: compressedBytes },
      ratio: originalBytes / (compressedBytes || 1),
      percentage: 100 - (compressedBytes / originalBytes) * 100,
      techniques: [],
    };
  }
  ...
  private createCompressedContext(
    ...
    const metrics = this.calculateMetrics(compressed, original);
